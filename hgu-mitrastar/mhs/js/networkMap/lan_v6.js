...
function lanv6statusmodified(check){
    if (check.checked) {
        setTimeout("activedLANv6();", 0);
    }
    else {
        if (lanv4warningsactived) {
            showInfoDialog($("#wifi_alert1").html(), false, true, function(resp){
                if (resp == true) 
                    setTimeout('deactivedLANv6();', 0);
                else {
...
function activedLANv6(){
    $("#lanv6Conf").css('display', 'block');
    //load_iphone_butons();
    $("#lanv6Conf").show('blind', 500, function(){
    });
    $("#wifi .wifi_ico").attr("color", "green");
}

function deactivedLANv6(){
    $("#lanv6Conf").hide('blind', 500, function(){
    });
...
function lanv6accept(){
    ...
    var LANv6JSON = prepareLANv6JSON();
    setLANv6Configuration(LANv6JSON, function(success, error_desc){
        if (!success) 
