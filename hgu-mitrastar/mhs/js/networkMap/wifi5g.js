...
function prepareWifi5gJSON(){
    var key = new Object();
	key.sessionKey=$("#sessionKey").val();
    var wifi = new Object();
    wifi.status = 1;
	//wifi.macfltmode = "2"; //Permitir
    ...
    var wps = new Object();
    if (wifi.ssidVisibility==1) {
        wps.status = 'disabled';
    }
    else if (wifi.ssidVisibility==0){
        wps.status = 'configured';
    }
    else {
        var wpsstatus = $("#wps_status").attr("value");
        wps.status = wpsstatus;
    }
...
function wifi5gaccept(){
	...
	if ($("#wifi_sec")[0].value == "WEP-64Bits")
	{
		...
		{
			alert("The key must have either 5 or 10 characters.");
			return false;
		}
	}
	else if ($("#wifi_sec")[0].value == "WEP-128Bits")
	{
		...
		{
			alert("The key must have either 13 or 26 characters.");
			return false;
		}
		else if ($("#wifiPass").attr("value").length == 26 )
		{
			...
			if(!isValidASCII($("#wifiPass").attr("value")))
			{
				return false;
			}
		}
...
*/		
	...
	if (resultwpa == true) {	
       ...
       // if (test == true) {
            var wifiJSON = prepareWifi5gJSON();
            setWifi5gConfiguration(wifiJSON, function(success, error_desc){
                if (!success) 
...
/*		
		...
		for(i=0;i<wpapsklen;i++){
			var c=value.charAt(i);
			if(doHexCheck(c)<0){
				$('#content').text("Pre-Shared longitud error clave!");
				$('#tab1').fadeIn();
				return true;
			}
		}
