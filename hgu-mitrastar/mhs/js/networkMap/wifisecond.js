...
function prepareWifiSecondJSON(){
    var key = new Object();
	key.sessionKey=$("#sessionKey").val();
    var wifi = new Object();
    wifi.status = 1;
	
	if($("#EnbSwitch").attr("checked"))
		wifi.enableWireless = "1";
	else
		wifi.enableWireless = "0";
...
function wifisecondaccept(){
    wifiwarningsactived = false;
	var resultwpa = true;
    renoveSession();

	if ($("#EnbSwitch").checked == true) {
...
*/		
	...
	if (resultwpa == true) {	
		var wifiJSON = prepareWifiSecondJSON();
		setWifiSecondConfiguration(wifiJSON, function(success, error_desc){
			if (!success) 
